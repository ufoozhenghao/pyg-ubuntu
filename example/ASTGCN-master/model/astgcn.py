...
class ASTGCN_block(nn.Block):
    ...
    def forward(self, x):
        ...
        x_TAt = nd.batch_dot(x.reshape(batch_size, -1, num_of_timesteps), temporal_At).reshape(batch_size,
                                                                                               num_of_vertices,
                                                                                               num_of_features,
                                                                                               num_of_timesteps)
    ...
    def __init__(self, num_for_prediction, all_backbones, **kwargs):
        ...
        with self.name_scope():
            for backbones in all_backbones:
                ...
                # 注册这个子模块，使其成为当前模块（即 ASTGCN 实例）的子模块。这一步非常重要，因为它确保了这个子模块的参数会被正确管理和更新
